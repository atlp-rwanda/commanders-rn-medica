...
export default function SetFingerPrint() {
    ...
    const handleAuth= async()=>{
        const result = await LocalAuthentication.authenticateAsync({
            promptMessage: "Please put your finger on the fingerprint scanner to get started."
        })
        if(result.success){
            alert("success");
        }
    ...
    return (
      ...
      >
        <View className="flex-1 justify-center items-center" style={{backgroundColor:"#09101D70"}}>
          <View className="justify-center items-center p-4 gap-y-6 bg-white w-5/6 rounded-2xl">
            <Image
              source={require("@/assets/icons/userprofile/confirmprofile.png")}style={{width:180, height:150}}
              resizeMode="contain"
            />
            <Text className="font-UrbanistSemiBold text-xl text-darkblue">Congratulations</Text>
            <Text className="font-UrbanistRegular text-center">
              Your account is ready to use. You will be redirected to the Home
              page in a few seconds.
            </Text>
            
              <Animated.Image style={{transform:[{rotate: spin}],width:50, height:50}}
