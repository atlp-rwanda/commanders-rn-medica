...
export default function SelectPackageScreen() {
	...
	const submitPackage = () => {
		const SelectedCommunication = packages.find(
			(pkg) => pkg.key === selectedPackage
		);
		const SelectedTimeDuration = durations.find(
			(dur) => dur.key === selectedDuration
		);
		...
		router.push({
			pathname: "/doctor-appointments/patient-details",
			params: {
				date,
				time,
				packageTitle,
